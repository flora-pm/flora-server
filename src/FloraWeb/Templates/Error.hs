      ...
      main_ [class_ "sm:flex"] $ do
        ...
        div_ [class_ "sm:ml-6"] $ do
          div_ [class_ "sm:border-l sm:border-gray-200 sm:pl-6"] $ do
            h1_ [class_ "text-6xl font-extrabold dark:text-white text-gray-900 tracking-tight sm:text-5xl"] $
              toHtml $ statusMessage status
          div_ [class_ "mt-10 flex space-x-3 sm:border-l sm:border-transparent sm:pl-6"] $ do
            a_ [href_ "/", class_ "inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm dark:text-white text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 error-page-button"] "Go back home"
