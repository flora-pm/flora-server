  ...
  where
    ...
    displayVersion release =
      li_ [class_ "release"] $ do
        ...
        a_
          ([class_ versionClass, href_ $ versionResource namespace packageName release.version] <> dataText)
...
showAll target mVersion namespace packageName = do
  let resource = case target of
        Dependents -> packageResource namespace packageName <> "/dependents?page=1"
        Dependencies -> versionResource namespace packageName (fromJust mVersion) <> "/dependencies"
        Versions -> packageResource namespace packageName <> "/versions"
            ...
            ul_ [class_ "package-alternatives"] $
              Vector.forM_ inFavourOf $
                \PackageAlternative{namespace, package} ->
                  li_ [] $
                    a_
                      [href_ $ packageResource namespace package]
  ...
  li_ [class_ ""] $ do
    ...
    div_ [class_ "items-top"] $ case mLatestViableRelease of
      ...
      Just (namespace, package, version) -> do
        ...
        a_
          [href_ $ versionResource namespace package version]
...
renderDependent Package{name, namespace} = do
  ...
  a_ [class_ "dependent", href_ $ packageResource namespace name] qualifiedName

renderDependency :: (Namespace, PackageName, Text) -> FloraHTML
renderDependency (namespace, name, version) = do
  li_ [class_ "dependency"] $ do
    a_ [href_ $ packageResource namespace name] (toHtml name)
    toHtmlRaw @Text "&nbsp;"
    if version == ">=0"
      then ""
      else toHtml version
...
versionResource namespace name version = packageResource namespace name <> "/" <> display version
