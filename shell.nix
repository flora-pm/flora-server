...
with pkgs; rec {
  ...
  flora = haskell.lib.addTestToolDepend ((haskellPackages.override {
    overrides = self: super: with haskell.lib; rec {
      PyF = super.callHackage "PyF" "0.10.2.0" { };
      lucid = super.callHackage "lucid" "2.11.0" { };
      lucid-alpine = doJailbreak (super.callHackage "lucid-alpine" "0.1.0.4" {});
      ...
      wai-middleware-heartbeat = super.callCabal2nix "wai-middleware-heartbeat"
        (fetchTarball {
  ...
  { }) postgresql;
  newShell = flora.env.overrideAttrs (old: {
    buildInputs = old.buildInputs ++ [ cabal-install haskellPackages.haskell-language-server ];
  });
}
