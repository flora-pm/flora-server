create materialized view latest_versions (
  ...
  package_id,
  version) as
    select distinct p.namespace, p.name, r.metadata ->> 'synopsis' as synopsis
                  , p.package_id, max(r.version) as version
    from "packages" as p
      inner join "releases" as r on r."package_id" = p."package_id"
    group by (p.namespace, p.name, r.metadata ->> 'synopsis', p.package_id);
