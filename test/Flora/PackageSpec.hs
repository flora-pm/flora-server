...
spec = describeDB migrate "packages" $ do
  ...
  itDB "Fetch the dependants of ghc-prim" $ do
    result <- Set.fromList . Vector.toList <$> getPackageDependants (array ^. #namespace) (ghcPrim ^. #name)
    result `shouldBe` Set.fromList [base, ghcBignum, deepseq, bytestring, integerGmp, binary]
  itDB "Fetch the dependents of array" $ do
    result <- Set.fromList . Vector.toList <$> getPackageDependants (array ^. #namespace) (array ^. #name)
